...
export default function Projects() {
  return (
    <section id="projects" className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
      <h2 className="font-mono text-2xl md:text-3xl font-bold mb-6 sm:mb-10">{"//"} featured projects</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {projects.map((project) => (
          <div
            key={project.name}
            className="border-3 border-black bg-white p-4 sm:p-6 shadow-[4px_4px_0_#000] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all duration-150 flex flex-col"
          >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {project.stack.map((tech) => (
                <span
                  key={tech}
                  className="font-mono text-[10px] sm:text-xs px-2 py-1 bg-gray-100 border border-black/20"
                >
                  {tech}
                </span>
              ))}
            ...
            <div className="flex gap-4 sm:gap-3">
              {project.repo && (
                <a
                  ...
                  className="font-mono text-sm font-bold hover:text-accent transition-colors inline-flex items-center gap-1 py-2 sm:py-0"
                >
